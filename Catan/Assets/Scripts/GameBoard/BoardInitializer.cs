...
tileTypes: ћассив типов тайлов, которые будут использоватьс€ на доске.
tileAmount: ћассив, определ€ющий количество тайлов каждого типа.
boardShape: ћассив, определ€ющий форму доски.
diceValues: ћассив значений кубика дл€ каждого тайла.
ports: ћассив типов портов, которые могут присутствовать на доске.
portAmounts: ћассив, определ€ющий количество портов каждого типа.
ћетоды класса:

Initialize: ћетод дл€ инициализации игровой доски. ќн загружает предустановленные параметры, случайным образом создает тайлы на доске, инициализирует вершины, дороги и порты, а затем размещает их на доске.
Reinitialize: ћетод дл€ повторной инициализации доски. ќн очищает текущее состо€ние доски и повторно инициализирует ее.
Randomize: ћетод дл€ случайного создани€ тайлов на доске в соответствии с заданными параметрами.
InitializeVertices: ћетод дл€ инициализации вершин доски.
InitializePorts: ћетод дл€ инициализации портов на доске.
BuildPortPerimeter: ћетод дл€ построени€ периметра портов на доске.
InitializeRoads: ћетод дл€ инициализации дорог на доске.
GetRandomTileType: ћетод дл€ получени€ случайного типа тайла из доступных.
GetRandomDiceValue: ћетод дл€ получени€ случайного значени€ кубика дл€ тайла.
GetRandomPortType: ћетод дл€ получени€ случайного типа порта из доступных.
LoadPreset: ћетод дл€ загрузки предустановленных параметров доски из настроек игры.
*/
